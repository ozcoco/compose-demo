    ...
    LaunchedEffect(vm) {
        ...
        vm.effect.collect { effect ->
            when (effect) {
                ...
                MainEffect.UseLocalModelChange -> {
                    // 提示模型切换
                    scope.launch(Dispatchers.Main) {
                        Toast.makeText(
                            context,
                            if (state.isUseLocalModel) "已切换到本地模型" else "已切换到在线模型",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
                    ...
                    label = {
                        Row(
                            ...
                            Switch(
                                ...
                                checked = state.isUseLocalModel,
                                onCheckedChange = {
                                    onIntent(MainIntent.UseLocalModel(it))
                                })
                        }
                    },
                    selected = false,
